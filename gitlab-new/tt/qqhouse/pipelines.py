...
class QqhousePipeline(object):
	...
	def init_self_attributes(self, spider):
		...
		if self.csv_file_path is None or 1 > len( self.csv_file_path ):
			self.csv_file_path = os.path.join( self.crawled_dir, f"qqhouse{self.overwrite_today}.csv" )
	...
	def close_spider(self, spider = None):
		...
		if spider is not None:
			spider.logger.info( f"Method close_spider of Class QqhousePipeline is called at {now}" )
