...
class Handlers
{
	...
	*/
	...
	static var cus_rootPath: String = "C:\\Entrobus\\projects\\crawl\\20190214scrapy\\tt\\dianping\\";
	// static var crawling_city = "guangzhou";
	// static var crawling_city = "shenzhen";
	// static var crawling_city = "foshan";
	static var crawling_city = "yueyang";
	// static var crawling_city = "zhuzhou";
	static var shopid_counter: int = 0;
	static var shopid_list = [];
	...
	*/
	...
	static function get_date_time(){
		...
		month = (Array(2).join(0) + (month + 1) ).slice(-2);
		...
		var formatted = [ hour, minute, second ].join("");
		formatted = [ formatted_date, formatted ].join("_");
		return {
			date: formatted_date,
			datetime: formatted,
			timestamp: date_obj.valueOf()
		}
	}

	static function SaveToLocalFile( content: String, root_path: String, filename:String ) {
		var date_obj = new Date();
		var timestamp = date_obj.valueOf();
		if ( 1 > root_path.length ){
			root_path = cus_rootPath;
		}
		if ( 1 > filename.length ){
			filename = timestamp
		}
		var txt_filename = [ root_path, filename ].join("");
		var writer = new StreamWriter(txt_filename, true, new UTF8Encoding(false) )
		// https://docs.microsoft.com/zh-cn/dotnet/api/system.io.streamwriter?view=netframework-4.7.2
		// 20190329 added the following line for fixing a bug!
		writer.AutoFlush = true;
		writer.WriteLine( content );
	}

	static function SaveResponseBodyToLocalFile(oSession: Session, use_suggested:Boolean, root_path: String, filename:String, suffix:String) {
		var date_obj = new Date();
		var timestamp = date_obj.valueOf();
		if ( 1 > root_path.length ){
			root_path = cus_rootPath;
		...
		if ( use_suggested ){
			oSession.SaveResponseBody( [ root_path, filename, "_", oSession.SuggestedFilename ].join("") );
		}else if ( 0 < suffix.length ) {
			oSession.SaveResponseBody( [ root_path, filename, suffix ].join("") );
		} else {
			oSession.SaveResponseBody( [ root_path, filename ].join("") );
		}
	}

	static function ReadDataFile(filename:String, root_path: String ){
		var fso;
		var f;
		var content = "";
		if ( 1 > filename.length ){
		...
		var file_path = [ root_path, filename ].join("")
		...
		fso = new ActiveXObject("Scripting.FileSystemObject");
		f = fso.OpenTextFile( file_path, 1 );
		return( f.ReadAll() );
	}
	...
	static function OnBeforeResponse(oSession: Session) {
		...
		// the followings are added by Peter Yu on 20190321
		var url, pos, css_pos, filename, root_path, html_filename, ret_obj, ch_level2, channel, level2, pos2;
		var match_str = ["www.dianping.com/", crawling_city].join("");

		// On 20190325 add shop detailed pages
		var match_shop = "www.dianping.com/shop/";
		var shop_id;

		oSession.utilDecodeResponse(); // decode response
		try{
			if ( 200 == oSession.responseCode ){
				url = oSession.url;
				if ( oSession.fullUrl.Contains("//s3plus.meituan.net") ){
					pos = url.lastIndexOf( ".svg" )
					css_pos = url.lastIndexOf( ".css" )
					if (-1 < pos || -1 < css_pos){
						if (-1 < css_pos){
							pos = css_pos;
						}
						// var suffix = url.slice(pos + 1, url.length)
						pos = url.lastIndexOf( "/" )
						filename = url.slice(pos + 1, url.length)
						root_path = [ cus_rootPath, "svgtextcss_fiddler\\" ].join("")
						Handlers.SaveResponseBodyToLocalFile(oSession, false, root_path, filename, "");
					}
				} else if ( oSession.fullUrl.Contains( match_str ) ){
					ret_obj = Handlers.get_date_time();
					pos = url.indexOf( match_str );
					pos2 = url.indexOf( "?" );
					if ( -1 < pos2 ){
						url = url.substr(0, pos2);
					}
					ch_level2 = url.slice( pos + match_str.length, url.length );
					pos = ch_level2.lastIndexOf( "/" );
					channel = ch_level2.slice( 1, pos );
					level2 = ch_level2.slice( pos + 1, ch_level2.length );
					html_filename = [ crawling_city, channel, level2, ret_obj.date ].join("_");
					html_filename = [ html_filename, ".html" ].join("");
					root_path = [ cus_rootPath, "list_html_fiddler\\" ].join("")
					Handlers.SaveToLocalFile( oSession.GetResponseBodyAsString(), root_path, html_filename )
				} else if ( oSession.fullUrl.Contains( match_shop ) ){
					ret_obj = Handlers.get_date_time();
					pos = url.indexOf( match_shop );
					shop_id = url.slice( pos + match_shop.length, url.length );
					html_filename = [ "shop", shop_id, ".html" ].join("");
					root_path = [ cus_rootPath, "detail_html_fiddler\\" ].join("")
					Handlers.SaveToLocalFile( oSession.GetResponseBodyAsString(), root_path, html_filename )
				}
			}
		} catch(e) {
			FiddlerApplication.Log.LogString( [ "cannot SaveResponseBodyToLocalFile. e = ", e ].join("") );
		}
	}
	...
	*/
	...
	static function Main() {
		...
		UI.lvSessions.AddBoundColumn("HostIP",50,"x-hostIP");
