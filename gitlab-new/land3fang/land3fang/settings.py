...
from . import settings_land3fang
settings.setmodule( settings_land3fang )

# imported = False
# module_name = ""
# settings_dir = os.path.dirname(os.path.abspath(__file__)) # C:\Entrobus\projects\crawls\land3fang\land
# for one in os.listdir( settings_dir ):
# 	file_path = os.path.join( settings_dir, one )
# 	if os.path.isfile( file_path ) and type( one ) in [str] and -1 < one.find("settings_"):
# 		module_name = one.replace(".py", "")
# 		module_name = "land.settings_land"
# 		__import__( module_name )
		# from . import eval( module_name )
		# eval( f"from . import {module_name}" )
		# eval(from . import module_name)
# 		settings.setmodule( module_name )
# 		imported = True
# if not imported:
# 	print( f"cannot import module_name({module_name})" )
# 	sys.exit(-100)
